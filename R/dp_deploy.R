...
dp_deploy <- function(project_path = ".", ...){
  ...
  d <- readRDS(file = glue::glue("{project_path}/output_files/RDS_format/",
                                 "data_object.RDS"))
  
  dp_deployCore(conf = conf, project_path = project_path, d = d, dlog = dlog, 
                git_info = git_info, ...)
...
dp_deployCore.labkey_board <- function(conf, project_path, d, dlog, git_info, 
  ...
  ver_current <- pins::pin_versions(name = as.character(attr(d, "dp_name")),
...
dp_deployCore.s3_board <- function(conf, project_path, d, dlog, git_info, 
  ...
  if (aws_creds$key == "" | aws_creds$secret == "") {
    if(aws_creds$profile_name == "")
      stop(cli::format_error("Please check aws credentials. You need to ",
