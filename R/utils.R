...
gitinfo_validate <- function(project_path, verbose = F) {
  ...
  git_info_valid <-
    nchar(git_sha <- as.character(last_commit$sha)) > 0 &
    nchar(git_uname <- as.character(last_commit$author$name)) > 0 &
    nchar(git_uemail <-
            as.character(last_commit$author$email)) > 0 &
    nchar(git_timestamp <-
            paste0(last_commit$author$when, collapse = " ")) > 0
  ...
  remote_url <-
    try(git2r::remote_url(repo = ".", remote = git2r::remotes()),
        silent = T)
...
#' @return TRUE
#' @keywords internal
dpboardlog_update <- function(conf,
                              git_info,
                              dlog = NULL,
                              dp_name = character(0),
    ...
    error = function(er) {
      ...
      if (length(dlog) == 0) {
        ...
        dpboard_log_tmp <- dpboard_log %>%
          dplyr::filter(
            .data$dp_name != dp_name | .data$pin_version != pin_version |
      ...
      tmp <- dpboard_log %>%
        dplyr::filter(
          .data$dp_name != daap_log_i$dp_name |
...
get_dlog <- function(project_path) {
  dlog <-
    yaml::read_yaml(file = glue::glue("{project_path}/.daap/daap_log.yaml"))
...
dpconnect_check <- function(board_params) {
  board_info <-
    try(pins::board_get(name = board_params$board_alias), silent = T)
