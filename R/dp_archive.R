...
dp_archive <- function(project_path = ".", ...){
  ...
  if(!is_valid_dp_repository(path = project_path))
    stop(cli::format_error(glue::glue("project_path, {project_path},",
  ...
  dp_archiveCore(conf = conf, dp_name = dp_name, pin_version = pin_version,
                 git_info = git_info, ...)
}
...
dp_archiveCore.labkey_board <- function(conf, dp_name, pin_version,
  ...
  if(verbose)
    print(glue::glue("Archiving {dp_name}, version {pin_version} from Labkey ",
                     "remote"))
  ...
  # Update dpboard_log
  dpboardlog_update(conf = conf, git_info = git_info, dp_name = dp_name,
...
dp_archiveCore.s3_board <- function(conf, dp_name, pin_version,
                                    git_info, verbose = F, ...){
  if(verbose)
    print(glue::glue("Archiving {dp_name}, version {pin_version} ",
                     "from S3 remote"))
  ...
  if (aws_creds$key == "" | aws_creds$secret == "") {
    if(aws_creds$profile_name == "")
      stop(cli::format_error(glue::glue("Please check aws credentials. You ",
